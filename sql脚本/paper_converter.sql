...
-- ----------------------------
-- Table structure for content_type_style
-- ----------------------------
DROP TABLE IF EXISTS `content_type_style`;
CREATE TABLE `content_type_style` (
  `id` int(11) NOT NULL,
...
-- ----------------------------
-- View structure for get_feedback_count
-- ----------------------------
DROP VIEW IF EXISTS `get_feedback_count`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `get_feedback_count` AS select `feedback_post`.`mail` AS `mail`,count(0) AS `mail_count` from `feedback_post` group by `feedback_post`.`mail` ;
